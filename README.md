...
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="docs/charts/detections_dark.png">
  <img alt="Real detections on the bundled sample data: STL-residual anomalies and a telemetry gap on server-room temperature; CUSUM level shifts bracketing a bad deploy on daily CPU means" src="docs/charts/detections_light.png">
</picture>

*Both panels are generated by the library itself — the anomaly markers, gap band, and changepoint segments are real outputs of `detect_anomalies`, `data_quality`, and `detect_changepoints` on the seeded sample datasets ([regenerate them](examples/make_readme_charts.py)).*
